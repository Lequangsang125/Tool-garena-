...
export async function getPrelogin() {
    let config = {
        ...
        url: `https://sso.garena.com/api/prelogin?app_id=10100&account=${info.username}&format=json&id=${new Date().getTime()}`,
        headers: {
...
export async function getLogin(password) {
    let config = {
        method: "get",
        maxBodyLength: Infinity,
        url: `https://sso.garena.com/api/login?app_id=10100&account=${info.username
...
export async function run() {
  try {
    const preloginData = await getPrelogin();
    ...
    const { data, ssoKey } = await getLogin(finalPassword);
